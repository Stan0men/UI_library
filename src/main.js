...
document.addEventListener("DOMContentLoaded", function() {
  sliders.forEach(s => {
    ...
    thumb.onmousedown = function(e) {
      ...
      if (s.getElementsByClassName("vo_slider_thumb_label").length > 0) {
        s.getElementsByClassName("vo_slider_thumb_label")[0].classList.add(
          "vo_slider_thumb_label_focus"
        );
  ...
  document.onmouseup = function(e) {
    sliders.forEach(s => {
      ...
      thumb.classList.remove("vo_slider_thumb_focus");
      if (s.getElementsByClassName("vo_slider_thumb_label").length > 0) {
        s.getElementsByClassName("vo_slider_thumb_label")[0].classList.remove(
          "vo_slider_thumb_label_focus"
        );
        thumb.classList.remove("vo_slider_thumb_focus");
...
document.addEventListener("mousemove", function(e) {
  if (e.which === 1) {
    ...
    timeout = setTimeout(() => {
      ...
      const thumbContainer = slider.getElementsByClassName(
        "vo_slider_thumb_container"
      )[0];
      ...
      if (slider.getElementsByClassName("vo_input_value").length > 0) {
        slider.getElementsByClassName("vo_input_value")[0].value = Math.round(
          100 * slider.value
        );
      }
      if (
        slider.getElementsByClassName("vo_slider_thumb_label_text").length > 0
      ) {
        slider.getElementsByClassName(
          "vo_slider_thumb_label_text"
        )[0].innerText = Math.round(100 * slider.value);
      }

      thumbContainer.style = `transform: translateX(-${100 -
        100 * slider.value}%);`;

      const sliderLeft = slider.getElementsByClassName(
        "vo_slider_track_background"
      )[0];
      const sliderRight = slider.getElementsByClassName(
        "vo_slider_track_fill"
      )[0];
